...
import LangSwitcher from "@/components/LangSwitcher";   // Импорт кнопок
...
const T = {
  ru: {
    ...
    disclaimerTitle: "Юридические оговорки и защита от ответственности",
    disclaimer1: "1. Статус сервиса: Adal Qadam является информационно-аналитическим инструментом. Сервис не оказывает юридическую помощь в смысле, определённом Законом РК.",
    disclaimer2: "2. Отказ от ответственности: Прогноз носит справочный характер и не является юридическим заключением.",
    disclaimer3: "3. Ограничение: Сервис не несёт ответственности за убытки. Решение принимает пользователь.",
    disclaimer4: "4. Актуальность: Прогнозы основаны на базе судебных решений.",
    loading: "Загрузка...",
  ...
  kk: {
    ...
    audit: "🕵️‍♂️ Заңгер тексеруі",
    logout: "Шығу",
    saved: "Үнемделді",
    docs: "Құжаттар",
    historyEmpty: "Тарих бос",
    disclaimerTitle: "Құқықтық ескертулер және жауапкершіліктен бас тарту",
    disclaimer1: "1. Сервис мәртебесі: Adal Qadam — ақпараттық-талдау құралы. Сервис ҚР Заңына сәйкес заң көмегін көрсетпейді.",
    disclaimer2: "2. Жауапкершіліктен бас тарту: Болжам анықтамалық сипатта болады және заңды қорытынды емес.",
    disclaimer3: "3. Шектеу: Сервис шығындар үшін жауап бермейді. Шешімді пайдаланушы қабылдайды.",
    disclaimer4: "4. Өзектілік: Болжамдар сот шешімдерінің базасына негізделген.",
    loading: "Жүктелуде...",
    serverError: "Сервермен байланыс үзілді"
  },
  en: {
    chat: "AI Chat",
    ...
    disclaimerTitle: "Legal Disclaimers and Limitation of Liability",
    disclaimer1: "1. Service Status: Adal Qadam is an information and analytical tool. It does not provide legal aid as defined by RK Law.",
    disclaimer2: "2. Disclaimer: Predictions are for informational purposes only and are not a legal opinion.",
    disclaimer3: "3. Limitation: The service is not liable for losses. Final decisions rest with the user.",
    disclaimer4: "4. Accuracy: Predictions are based on a database of court rulings.",
    loading: "Loading...",
...
export default function Profile() {
  const { lang } = useLanguage(); // Глобальный язык
  const t = T[lang as keyof typeof T]; // Текущий перевод
  ...
  const handleChangeUsername = async () => {
    const promptText = lang === 'kk' ? "Жаңа логинді енгізіңіз (кемінде 3 таңба):" : 
                       lang === 'en' ? "Enter new username (min 3 chars):" : 
    ...
    try {
      ...
      if (res.ok) {
        alert(lang === 'kk' ? "Логин сәтті өзгертілді!" : lang === 'en' ? "Username changed!" : "Логин успешно изменен!");
        localStorage.removeItem('token');
        router.push('/login');
      } else {
  ...
  if (!user) return <div className="min-h-screen bg-[#080f1e] flex items-center justify-center text-blue-400">{t.loading}</div>;
          ...
          <div className="flex-1 overflow-y-auto px-4">
            {cases && cases.map((c: any) => (
              <button key={c?.id} onClick={() => router.push(`/?case=${c?.id}`)} className="w-full text-left px-4 py-3 rounded-xl text-[14px] truncate text-gray-400 hover:bg-blue-600/10 hover:text-blue-200 block mb-1">
                {c?.title || (lang === 'kk' ? "Атаусыз" : "Untitled")}
              </button>
            ))}
          </div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="space-y-4">
            {(!cases || cases.length === 0) ? (
              <p className="text-center text-gray-500 py-10">{t.historyEmpty}</p>
            ) : (
              [...cases].reverse().map((c: any) => (
                <div key={c?.id} className="bg-[#0f192e] border border-blue-500/20 p-6 rounded-2xl flex justify-between items-center hover:bg-blue-900/40 cursor-pointer" onClick={() => router.push(`/?case=${c?.id}`)}>
                  <div>
                    <p className="text-blue-100 font-medium">{c?.title || "Document"}</p>
                    <p className="text-gray-500 text-xs">
                        {c?.created_at ? new Date(c.created_at).toLocaleDateString(lang === 'kk' ? 'kk-KZ' : 'ru-RU') : ""}
                    </p>
                  </div>
          ...
          <div className="mt-12 bg-[#0c1527] border border-red-500/20 p-8 rounded-3xl relative overflow-hidden shadow-2xl">
            ...
            <div className="space-y-4 text-sm text-gray-400 leading-relaxed">
              <p><strong className="text-gray-300">{t.disclaimer1}</strong></p>
              <p><strong className="text-gray-300">{t.disclaimer2}</strong></p>
              <p><strong className="text-gray-300">{t.disclaimer3}</strong></p>
              <p><strong className="text-gray-300">{t.disclaimer4}</strong> <span className="text-blue-400 font-mono">{new Date().toLocaleDateString()}</span>.</p>
            </div>
          </div>
        </div>
      </main>
